...
public class PostsPage extends ListPage<PostsAdapter> implements FavoritesStorage.Observer, UiManager.Observer,
    ...
    /**
     * Если разница между первой видимой позицией и последней запомненной больше этого числа,
     * то будет использована первая видимая.
     */
    private final int SCROLL_POSTS_POSITION_DIFF = 3;

    /**
     * Сколько времени показывать сообщение в title ActionBar'а.
     */
    private final int SHOW_MESSAGE_IN_TITLE_TIME_MILLIS_DEFAULT = 1500;

    /**
     * Таймер возвращения оригинального title треда.
     */
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            if (postItem != null && !postItem.isHidden(hidePerformer)) {
                ...
                if (helper.hasIncluded()) {
                    QUERIES:
                    for (String lowQuery : helper.getIncluded()) {
                        if (comment.contains(lowQuery)) {
                            found = true;
                            break;
    ...
    /**
     * Получить отсортированный лист с позициями постов с заданными номерами.
     *
     * @param postNumbers Номера постов.
     * @return Отсортированный лист с позициями этих постов.
     */
    private ArrayList<Integer> getSortedPositionsOfPostNumbers(HashSet<String> postNumbers) {
        ArrayList<Integer> list = new ArrayList<>();
        for (String postNumber : postNumbers) {
            list.add(getAdapter().findPositionByPostNumber(postNumber));
        }
    ...
    /**
     * Вернуть предыдущую позицию по отношению к заданной или null, если такой нет.
     *
     * @param sortedPositions Отсортированный лист позиций постов.
     * @param somePosition    Любая позиция, даже та, которой нет в заданном листе.
     * @return Предыдущая позиция или null, если такой нет.
     */
    private Integer getPreviousPositionCycled(ArrayList<Integer> sortedPositions, int somePosition) {
        Integer previous = null;
        for (int position : sortedPositions) {
            if (position >= somePosition) {
                break;
            }
            previous = position;
        }

        // Если предыдущей позиции нет, но лист не пуст, возвращаем последнюю позицию листа - круговая зацикленность.
        if (previous == null && sortedPositions.size() != 0) {
            int lastPosition = sortedPositions.size() - 1;
            return sortedPositions.get(lastPosition);
        }
    ...
    /**
     * Вернуть следующую позицию по отношению к заданной или null, если такой нет.
     *
     * @param sortedPositions Отсортированный лист позиций постов.
     * @param somePosition    Любая позиция, даже та, которой нет в заданном листе.
     * @return Следующая позиция или null, если такой нет.
     */
    private Integer getForwardPositionCycled(ArrayList<Integer> sortedPositions, int somePosition) {
        for (int position : sortedPositions) {
            if (position > somePosition) {
                return position;
            }
        ...
        // Если следующей позиции нет, но лист не пуст, возвращаем первую позицию листа - круговая зацикленность.
        if (sortedPositions.size() != 0) {
            return sortedPositions.get(0);
        }
    ...
    /**
     * Получить предыдущую позицию при скроллинге постов.
     *
     * @return Предыдущая позиция.
     */
    private int getScrollPostsPreviousPosition() {
        int firstVisiblePosition = getListView().getFirstVisiblePosition();
        if (scrollPostsPreviousPosition == null) {
            scrollPostsPreviousPosition = firstVisiblePosition;
        }

        // Если разница между первой видимой и запомненной позицией меньше 5, то используем запомненную,
        // иначе - первую видимую.
        if (Math.abs(firstVisiblePosition - scrollPostsPreviousPosition) < SCROLL_POSTS_POSITION_DIFF) {
            return scrollPostsPreviousPosition;
        }
    ...
    /**
     * Проскролить посты относительно текущей позиции.
     *
     * @param sortedPositionsOfPosts Позиции нужных постов.
     * @param isPrevious             Проскроллить назад или вперед?
     * @return true
     */
    private boolean scrollPostsCycled(ArrayList<Integer> sortedPositionsOfPosts, boolean isPrevious) {
        int currentPosition = getScrollPostsPreviousPosition();
        Integer positionOfPost;
        if (isPrevious) {
            positionOfPost = getPreviousPositionCycled(sortedPositionsOfPosts, currentPosition);
        } else {
            positionOfPost = getForwardPositionCycled(sortedPositionsOfPosts, currentPosition);
        }
    ...
    /**
     * Получить ответы на заданные посты.
     * Порядок не гарантируется.
     *
     * @param postNumbers Номера постов.
     * @return Номера постов с ответами.
     */
    private HashSet<String> getRepliesToPostNumbers(HashSet<String> postNumbers) {
        HashSet<String> referencesToPosts = new HashSet<>();
        for (String postNumber : postNumbers) {
            PostItem post = getAdapter().findPostItem(postNumber);
            LinkedHashSet<String> referencesFrom = post.getReferencesFrom();
            if (referencesFrom != null) {
                referencesToPosts.addAll(referencesFrom);
            }
        }

        return referencesToPosts;
    }

    /**
     * Show message in the title of the ActionBar.
     *
     * @param msg Message.
     */
    private synchronized void showMessageInTitle(String msg) {
        String originalTitle = getPageHolder().threadTitle;

        if (restoreTitleTimer == null) {
            restoreTitleTimer = new AsyncTimer(SHOW_MESSAGE_IN_TITLE_TIME_MILLIS_DEFAULT, () -> {
                // Возвращаем оригинальный title.
                getActivity().runOnUiThread(() -> {
                    getPageHolder().threadTitle = originalTitle;
                    notifyTitleChanged();
                });
            });
        }
